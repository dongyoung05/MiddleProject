...
public class CartInsert extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("cart") != null) {
			...
			//	장바구니에 상품이 있는지 여부 확인
			ProdCartVO chkProd = new ProdCartVO();
			chkProd.setCart_no(cartNo);
			chkProd.setProd_no(prodNo);
			
			int chkprod = prodCartService.checkCartProd(chkProd);
			
			if(chkprod > 0) {
				//	중복인 경우
				//	중복된 상품의 정보가 담긴 VO 객체에 수량 정보도 추가
				chkProd.setProd_qty(prodQty);
				int cnt = prodCartService.duplicationCartProd(chkProd);
				
				//	결과 메서드 호출
				cntProcess(cnt, prodNo, request, response);
			} else {
				//	상품 장바구니 객체 생성
	...
	//	작업 성공 시 호출되는 메서드
	private void cntProcess(int cnt, String prodNo, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(cnt == 0) {
			//	장바구니에 물건 담기를 실패한 경우
			PrintWriter writer = response.getWriter();
			...
			//	View 페이지 이동
			request.getRequestDispatcher("/prod/view/resultProd.jsp").forward(request, response);
		}
	}
}
