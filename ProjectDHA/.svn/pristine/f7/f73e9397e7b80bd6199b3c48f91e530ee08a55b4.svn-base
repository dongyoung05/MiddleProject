...
$(function(){
	...
	$(document).on('click','.update', function(){
		...
		type = $(this).parents('.tr').find('td:eq(1)').text();
		title = $(this).parents('.tr').find('td:eq(2)').text();
		desc = $(this).parents('.tr').find('td:eq(3)').text();
		all = $(this).parents('.tr').find('td:eq(4)').text();
		day = $(this).parents('.tr').find('td:eq(5)').text();
		night = $(this).parents('.tr').find('td:eq(6)').text();
		$('#NO2').text(no);
		$('#TYPE2').val(type);
		$('#TITLE2').val(title);
		$('#DESC2').val(desc);
	...
	$('#ok2').on('click', function(){
		...
		all = $('#PRICE_ALL2').val();
		day = $('#PRICE_DAY2').val();
		night = $('#PRICE_NIGHT2').val();
		
		$.getJSON(
			"<%= request.getContextPath()%>/admin/updateRoom.do",
			{
				"no":no,
				"type":type,
				"title":title,
...
<div id="m2" class="modal_container" style="margin: none;">
	<div class="modal">
		...
		<div class="modal_content">
			...
			<p>
				<span class="modalSpan">TYPE</span>
				<span id="mmemName"><input type='text' id='TYPE2'></span>
			</p>
			<p>
				<span class="modalSpan">TITLE</span>
				<span id="mmemName"><input type='text' id='TITLE2'></span>
			</p>
			<p>
				<span class="modalSpan">DESC</span>
			...
			<p>
				<span class="modalSpan">PRICE_ALL</span>
				<span id="mmemName"><input type='number' id='PRICE_ALL2'></span>
			</p>
			<p>
				<span class="modalSpan">PRICE_DAY</span>
				<span id="mmemName"><input type='number' id='PRICE_DAY2'></span>
			</p>
			<p>
				<span class="modalSpan">PRICE_NIGHT</span>
				<span id="mmemName"><input type='number' id='PRICE_NIGHT2'></span>
			</p>
		</div>
		<hr>
		<div class="modal_footer">
